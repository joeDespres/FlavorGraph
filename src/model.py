...
def load_augmentive_features(nodes):
    ...
    print(device)
    return (
        torch.tensor(augmentive_matrix, requires_grad=False).float().to(device),
        vector_length,
        torch.tensor(binary_mask, requires_grad=False).float().to(device),
    )
...
class SkipGramModelAux(SkipGramModel):
    def __init__(
        self, emb_size, emb_dimension, nodes=None, aux_coef=0.0001, CSP_save=False
    ):
        super(SkipGramModelAux, self).__init__(emb_size, emb_dimension)
        self.emb_size = emb_size  # row / # of vocab size / 8298
        self.emb_dimension = (
            emb_dimension  # column / user-defined vector dimension / 128
        )
        self.aux_coef = aux_coef
        self.aux_loss = 0.0
        self.CSP_save = CSP_save

        self.aug_embeddings, self.aug_dimension, self.binary_masks = (
            load_augmentive_features(nodes)
        )
        ...
        self.a_embeddings.weight = nn.Parameter(
            self.aug_embeddings, requires_grad=False
        )
