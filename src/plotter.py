...
def plot_category(
    node2vec, node2vec_tsne, path, node2name, node2is_hub, withLegends=False
):
    # Label Load
    labels = []
    ...
    if withLegends:
        ...
        make_plot_only_labels(
            name=path, points=node2vec_tsne, labels=labels, publish=False
        )
...
def make_plot_with_labels_legends(
    name,
    points,
    labels,
    label_to_plot,
    ...
    for legend_label, group in itertools.groupby(full, lambda x: x[2]):
        ...
        traces.append(
            go.Scattergl(
                ...
                text=[
                    "{} ({})".format(label, pretty_legend_label(legend_label))
